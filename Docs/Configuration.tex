...
PXE boot is already supported on most firmware. Using the additional network boot
...
\begin{itemize}
  ...
  If specified this driver will generate Auxiliary boot entries. \medskip
  ...
  If specified, enroll the given certificate for HTTPS Boot. The certificate can be specified
  as a multi-line PEM value between double quotes.
  \texttt{OWNER\_GUID} is optional, and will default to all zeros if not specified.
...
\emph{Note}: The certificate options above are processed one at a time, in
