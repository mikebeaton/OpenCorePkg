...
Provides in-memory emulated NVRAM within OpenCore. This can be useful on systems with fragile
(e.g. \href{https://forums.macrumors.com/threads/opencore-on-the-mac-pro.2207814/page-425?post=30945127#post-30945127}{MacPro5,1})
...
\tightlist
  ...
  \item CTRL+Enter in the OpenCore bootpicker updates or creates \texttt{NVRAM/nvram.plist}
\end{itemize}
...
Variables allowed for loading and for saving with CTRL+Enter must be specified in \texttt{LegacySchema}.
...
\emph{Note}: The \texttt{nvram.plist} (and \texttt{nvram.fallback} if present) files must have a
...
entry is still present it should be manually selected (using just Enter, not CTRL+Enter). Once a \texttt{macOS Installer}
...
If not, it should be manually selected (at this point, CTRL+Enter is a good idea as any final remaining installion restarts
