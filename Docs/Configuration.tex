...
\item
  ...
  may be any one of the names listed for that driver in
  \texttt{SysReport/Drivers/DriverInfo.txt}.

  \emph{Note 2}: The relevant names are the image section name, driver component name
  and driver image path (short form).
  Not all of these names are present and accessible using UEFI protocols on all firmware.
  For instance, only the driver component name can be accessed for loaded images
  on MP5,1 firmware.
  Any loaded image without any of these names cannot be unloaded using this setting.

  \emph{Note 3}:  These values may also be found using the OpenShell \texttt{dh}
  command (e.g. \texttt{dh \{DriverNum\}}, \texttt{dh -d \{DriverNum\}} and
  \texttt{dh -v \{DriverNum\}}).

  \emph{Note 4}: The NVRAM \texttt{Lang} and \texttt{PlatformLang} variables
  ...
  unloading images stable across any changes in these variables. When using
  \texttt{dh} with firmware with multi-lingual support, these variables may
  affect the driver component names listed, and it  may be necessary to temporarily
  clear these variables in order to make the \texttt{dh} command show driver
  component names in their default language.
