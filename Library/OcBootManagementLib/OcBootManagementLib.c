  ...
  while (TRUE) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if ((Context->PickerCommand == OcPickerShowPicker) && !IsApplePickerSelection) {
        ...
        //
        // If launching in text, clear screen of previous console contents - e.g. from
        // builtin picker, log messages, or previous console tool - before loading the entry.
        // Otherwise, if coming from graphics mode, set to trigger a full screen clear if anybody,
        // but particularly macOS verbose boot, switches to text mode.
        // But if coming from text mode do not set uncontrolled, intentionally allowing initial
        // verbose mode text to run on after the picker.
        //
        if (OcConsoleControlGetMode () != EfiConsoleControlScreenText) {
          gST->ConOut->TestString (gST->ConOut, OC_CONSOLE_MARK_UNCONTROLLED);
        } else if (Chosen->LaunchInText) {
          gST->ConOut->TestString (gST->ConOut, OC_CONSOLE_MARK_UNCONTROLLED);
