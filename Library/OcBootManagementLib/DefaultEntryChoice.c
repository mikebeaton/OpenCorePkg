  ...
  )
{
  ...
  CONST OC_CUSTOM_BOOT_DEVICE_PATH     *CustomDevPath;
  CONST OC_ENTRY_PROTOCOL_DEVICE_PATH  *EntryProtocolDevPath;
  EFI_DEVICE_PATH_PROTOCOL             *ExtBootDevPath;
  ...
  //
  // Get final device path for external boot entries.
  //
  ExtBootDevPath = NULL;
  if ((Entry->Type == OC_BOOT_EXTERNAL_BOOT) && (Entry->ExternalBootGetDevicePath != NULL)) {
    ExtBootDevPath = Entry->DevicePath;
    Status         = Entry->ExternalBootGetDevicePath (Context, &ExtBootDevPath);
    if (EFI_ERROR (Status)) {
      ExtBootDevPath = NULL;
    } else {
      ExtBootDevPathSize = GetDevicePathSize (ExtBootDevPath);
    }
  }
  ...
  for (Index = 0; Index < BootOrderCount; ++Index) {
    ...
    if (ExtBootDevPath != NULL) {
      DevicePathSize = GetDevicePathSize (BootOptionDevicePath);
      if (DevicePathSize >= ExtBootDevPathSize) {
        MatchedEntry = CompareMem (BootOptionDevicePath, ExtBootDevPath, ExtBootDevPathSize) == 0;
      }
    } else {
  ...
  if (!MatchedEntry) {
    ...
    if (ExtBootDevPath != NULL) {
      DevicePathSize = ExtBootDevPathSize;
    ...
    if (ExtBootDevPath != NULL) {
      CopyMem ((UINT8 *)(LoadOption + 1) + LoadOptionNameSize, ExtBootDevPath, DevicePathSize);
    } else if (!Entry->IsCustom) {
  ...
  )
{
  ...
  //
  ASSERT ((BootEntry->Type & OC_BOOT_EXTERNAL_BOOT) == 0);
