  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    for (Index = 0; Index < ProtocolBufferCount; Index++) {
      ...
      if (CompareGuid (&gEfiDevicePathProtocolGuid, ProtocolBuffer[Index])) {
        DebugPrintDevicePath(DEBUG_INFO, "DUMP:   DevicePath", Interface);
      }
      if ((ProtocolBufferCount > 2) && CompareGuid (&gEfiPciIoProtocolGuid, ProtocolBuffer[Index])) {
        for (BarIndex = 0; BarIndex <= 5; BarIndex++) {
          Supports = 0;
          TempStatus = ((EFI_PCI_IO_PROTOCOL *)Interface)->GetBarAttributes(Interface, BarIndex, &Supports, NULL);
          DEBUG ((DEBUG_INFO, "DUMP: Bar %u supports 0x%x - %r\n", BarIndex, Supports, TempStatus));
        }

        Supports = 0;
        TempStatus = ((EFI_PCI_IO_PROTOCOL *)Interface)->Attributes(Interface, EfiPciIoAttributeOperationSupported, 0, &Supports);
        DEBUG ((DEBUG_INFO, "DUMP: PciIo supports 0x%x - %r\n", Supports, TempStatus));

        Current = 0;
        TempStatus = ((EFI_PCI_IO_PROTOCOL *)Interface)->Attributes(Interface, EfiPciIoAttributeOperationGet, 0, &Current);
        DEBUG ((DEBUG_INFO, "DUMP: PciIo current 0x%x - %r\n", Current, TempStatus));

        if (Supports == 0x71A && Current == 0x600) {
          Current = 0x780;
          TempStatus = ((EFI_PCI_IO_PROTOCOL *)Interface)->Attributes(Interface, EfiPciIoAttributeOperationSet, Current, NULL);
          DEBUG ((DEBUG_INFO, "DUMP: PciIo set current to 0x%x - %r\n", Current, TempStatus));
  ...
  )
{
  ...
  DumpHandlesForProtocol (&gEfiPciIoProtocolGuid, "PCI_IO", TRUE, FALSE);
