...
// https://github.com/tianocore/edk2/blob/master/MdeModulePkg/Library/UefiBootManagerLib/BmBoot.c#L2061-L2071
//
EFI_STATUS
EFIAPI
  ...
  )
{
  ...
  *DevicePath = BmExpandLoadFiles (ChosenEntry->DevicePath, Data, DataSize, DmgLoading);
  ...
  //
  if (*DataSize != 0) {
    Status = ExtractOtherUri (*DevicePath, ".dmg", ".chunklist", &OtherUri, FALSE);
    if (!EFI_ERROR (Status)) {
      GotDmgFirst = TRUE;
