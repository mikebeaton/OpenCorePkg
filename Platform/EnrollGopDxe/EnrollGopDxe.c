/** @file
  Enroll GOP found after driver binding into ConSplitter using hotplug.
  ...
  )
{
  ...
  for (Index = 0; Index < HandleCount; ++Index) {
    if (IsValidGop (HandleBuffer[Index])) {
  ...
  )
{
  ...
  IsPassword = CompareGuid (&gApplePasswordUIFileGuid, NameGuid);
  if (IsPicker || IsPassword) {
    ...
    if (IsPassword) {
      ...
      // TODO: Improve security by working out exactly what needs
  ...
  )
{
  ...
  if (mPrepareForUIApp && !mAlreadyPreparedForUIApp && Status == EFI_NOT_FOUND) {
    // DEBUG ((
    //   DEBUG_INFO,
    //   "OCDM: gST %p gBS %p gBS->CreateEventEx %p &gBS %p\n",
    //   gST,
    //   gBS,
    //   gBS->CreateEventEx,
    //   &gBS
